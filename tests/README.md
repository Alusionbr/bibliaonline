...
| `test_build_smoke.py` | `build.py` (integração) | Roda o build inteiro num dataset de fixture e verifica páginas geradas, navegação livro→capítulo, leitura de capítulo, ferramentas de estudo (grifar/anotar/exportar), links anterior/próximo, índice de busca externo e `sitemap.xml`. |
...
(coletados: build_smoke=7, build=12, expand_verses=24, fill_pt=8, gen_translit=8)

> O comportamento do `study.js` no navegador (grifar palavra/versículo, anotar,
> copiar/baixar) é validado manualmente com jsdom; os testes de build acima
> garantem que os ganchos (`data-ref`, página de anotações, script) são gerados.
